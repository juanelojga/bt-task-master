...
- [ ] 1.1 Scaffold Vite React TypeScript project using `npm create vite@latest . -- --template react-ts`
- [ ] 1.2 Enable `strict: true` in `tsconfig.json` and verify the dev server runs with `npm run dev`
- [ ] 1.3 Clean up boilerplate files (remove default Vite demo content, App.css, etc.)

## 2. Folder Structure

- [ ] 2.1 Create feature-based directory structure under `src/features/`
- [ ] 2.2 Create `src/features/map/` directory for map-related code
- [ ] 2.3 Create `src/features/store/` directory for zustand store code
- [ ] 2.4 Create shared directories: `src/lib/` (utilities), `src/types/` (shared type definitions)

## 3. Tailwind CSS Setup

- [ ] 3.1 Install Tailwind CSS and its peer dependencies (`tailwindcss`, `@tailwindcss/vite`)
- [ ] 3.2 Configure Tailwind in `vite.config.ts` with the Tailwind plugin
- [ ] 3.3 Add Tailwind directives to the main CSS entry file (`@import "tailwindcss"`)
- [ ] 3.4 Verify Tailwind utility classes work by applying a class in App.tsx

## 4. ESLint & Prettier Configuration

- [ ] 4.1 Replace default ESLint config with Flat Config (`eslint.config.js`)
- [ ] 4.2 Install and configure ESLint plugins: `eslint-plugin-react-hooks`, `eslint-plugin-react-refresh`, `@typescript-eslint/eslint-plugin`, `@typescript-eslint/parser`
- [ ] 4.3 Enable `@typescript-eslint/recommended-type-checked` rule set
- [ ] 4.4 Set `@typescript-eslint/no-explicit-any` to `error`
- [ ] 4.5 Install Prettier and create `.prettierrc` configuration file
- [ ] 4.6 Install `eslint-config-prettier` and add it to ESLint config to disable conflicting rules
- [ ] 4.7 Add `lint` script to `package.json` and verify ESLint runs cleanly
